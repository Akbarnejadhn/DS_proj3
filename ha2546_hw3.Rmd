...
It can be observed that `r glm_auc`. We usually consider AUC of 0.8 or higher as good, so this value is pretty low and shows that the classification model is not performing very well.
...
Doing so, it can be observed that neither lag1 or lag2 are significant anymore when fitting glm. Also the ROC curve above shows `r roc_glm2_auc`, which is also not very good when predicting the direction od 2009 and 2010 using data from 1990 to 2008. 
...
As above ROC curve shows, we get `r lda_auc` using LDA, which is pretty similar to AUC value using Glm method.
...
As above ROC curve shows, we get `r qda_auc` using QDA, which is pretty similar to AUC value using LDA and Glm methods.
...
Comparing models built using *caret()*, we can observe that **KNN** model has the best performance with AUC of 0.565. However, the AUC values for other models do not have a considerable difference with values of 0.557 for LDA, 0.556 for Glm, and 0.529 for QDA. Also, even KNN with the highest value, does not have a high value for this model to be considered a perfect classifying model. So, we can conclude that the predictive ability of all these models are not very different from each other.